...
function encodeField(properties) {
  ...
  if (types2[type6] !== void 0)
    return [
      { type: "bytes32" },
      keccak2562(encodeData({ data: value, primaryType: type6, types: types2 }))
    ];
  if (type6 === "bytes") {
...
function pad2(bytes, options = {}) {
  ...
  for (let i2 = 0; i2 < size6; i2++) {
    ...
    paddedBytes[padEnd ? i2 : size6 - i2 - 1] = bytes[padEnd ? i2 : bytes.length - i2 - 1];
  }
  return paddedBytes;
}
...
function toString3(bytes, options = {}) {
  const { size: size6 } = options;
  let bytes_ = bytes;
  if (typeof size6 !== "undefined") {
    assertSize(bytes_, size6);
...
function keccak2563(value, options = {}) {
  ...
  const bytes = keccak_256(from7(value));
  if (as === "Bytes")
    return bytes;
...
function ripemd1603(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = ripemd1602(from7(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes2(bytes);
}
function sha2565(value, options = {}) {
  const { as = typeof value === "string" ? "Hex" : "Bytes" } = options;
  const bytes = sha2564(from7(value));
  if (as === "Bytes")
    return bytes;
  return fromBytes2(bytes);
}
...
function toBytes4(publicKey, options = {}) {
  return fromHex4(toHex5(publicKey, options));
}
...
function validate5(publicKey, options = {}) {
  try {
  ...
  constructor({ publicKey }) {
    super(`Value \`${publicKey}\` is an invalid public key size.`, {
      metaMessages: [
        ...
        `Received ${size4(from6(publicKey))} bytes.`
      ]
    });
    Object.defineProperty(this, "name", {
      enumerable: true,
...
function checksum2(address) {
  ...
  const hash9 = keccak2563(fromString4(hexAddress), { as: "Bytes" });
  const characters = hexAddress.split("");
  for (let i2 = 0; i2 < 40; i2 += 2) {
    if (hash9[i2 >> 1] >> 4 >= 8 && characters[i2]) {
      characters[i2] = characters[i2].toUpperCase();
    }
    if ((hash9[i2 >> 1] & 15) >= 8 && characters[i2 + 1]) {
...
function from9(address, options = {}) {
...
function fromPublicKey(publicKey, options = {}) {
  ...
  return from9(`0x${address}`, options);
}
...
var Solidity_exports2 = {};
__export(Solidity_exports2, {
...
function decodeAddress(cursor, options = {}) {
  ...
  return [wrap(fromBytes2(slice4(value, -20))), 32];
}
...
function decodeBytes(cursor, param, { staticPosition }) {
  ...
  const value = fromBytes2(cursor.readBytes(Number.parseInt(size6), 32));
  return [value, 32];
}
...
function decodeString(cursor, { staticPosition }) {
  ...
  const value = toString3(trimLeft2(data));
  cursor.setPosition(staticPosition + 32);
  return [value, 32];
}
...
function encodeBytes(value, { type: type6 }) {
  ...
  if (!parametersize) {
    ...
    return {
      dynamic: true,
      encoded: concat4(padLeft2(fromNumber2(bytesSize, { size: 32 })), value_)
    };
  }
...
function encodeString(value) {
  const hexValue = fromString3(value);
...
function encodeTuple(value, options) {
  ...
  return {
    dynamic,
    encoded: dynamic ? encode3(preparedParameters) : concat4(...preparedParameters.map(({ encoded }) => encoded))
  };
}
...
function decode(parameters, data, options = {}) {
  ...
  const bytes = typeof data === "string" ? fromHex4(data) : data;
...
(function(encodePacked3) {
  function encode11(type6, value, isArray = false) {
    ...
    if (arrayMatch && Array.isArray(value)) {
      ...
      const data = [];
      for (let i2 = 0; i2 < value.length; i2++) {
        data.push(encode11(childType, value[i2], true));
      }
      if (data.length === 0)
        return "0x";
      return concat4(...data);
...
function encode5(abiConstructor, options) {
  var _a;
  const { bytecode, args } = options;
  return concat4(bytecode, ((_a = abiConstructor.inputs) == null ? void 0 : _a.length) && (args == null ? void 0 : args.length) ? encode4(abiConstructor.inputs, args) : "0x");
}
...
function encode6(abiError, ...args) {
  const selector = getSelector2(abiError);
  const data = args.length > 0 ? encode4(abiError.inputs, args[0]) : void 0;
...
function assertArgs(abiEvent, args, matchArgs) {
  ...
  function isEqual4(input, value, arg) {
    ...
    if (input.type === "string")
      return keccak2563(fromString4(value)) === arg;
    if (input.type === "bytes")
...
function encode7(abiEvent, ...[args]) {
  let topics = [];
  if (args && abiEvent.inputs) {
    ...
    if (args_.length > 0) {
      const encode11 = (param, value) => {
        if (param.type === "string")
          return keccak2563(fromString3(value));
        if (param.type === "bytes")
          return keccak2563(value);
        if (param.type === "tuple" || param.type.match(/^(.*)\[(\d+)?\]$/))
          throw new FilterTypeNotSupportedError(param.type);
        return encode4([param], [value]);
      };
      topics = (indexedInputs == null ? void 0 : indexedInputs.map((param, i2) => {
        if (Array.isArray(args_[i2]))
          return args_[i2].map((_2, j) => encode11(param, args_[i2][j]));
        return typeof args_[i2] !== "undefined" && args_[i2] !== null ? encode11(param, args_[i2]) : null;
      })) ?? [];
    }
  }
  const selector = (() => {
...
function encodeData2(abiFunction, ...args) {
  ...
  const data = args.length > 0 ? encode4(item.inputs, args[0]) : void 0;
  return data ? concat4(selector, data) : selector;
}
function encodeResult(abiFunction, output, options = {}) {
  ...
  return encode4(abiFunction.outputs, values);
}
...
function fromTupleList(accessList) {
  ...
  for (let i2 = 0; i2 < accessList.length; i2++) {
    ...
    list.push({
      address,
      storageKeys: storageKeys.map((key) => validate4(key) ? key : trimLeft(key))
    });
  }
  return list;
}
...
async function encrypt2(value, key, options = {}) {
  ...
  const result = concat5(iv, new Uint8Array(encrypted));
  if (as === "Bytes")
    return result;
  return from6(result);
}
async function getKey(options) {
  ...
  const baseKey = await globalThis.crypto.subtle.importKey("raw", fromString4(password), { name: "PBKDF2" }, false, ["deriveBits", "deriveKey"]);
...
function decodeRlpCursor(cursor, to3 = "Hex") {
  if (cursor.bytes.length === 0)
    return to3 === "Hex" ? fromBytes2(cursor.bytes) : cursor.bytes;
  ...
  if (prefix < 192) {
    ...
    return to3 === "Hex" ? fromBytes2(bytes) : bytes;
  }
...
function fromBytes4(bytes, options = {}) {
  ...
  return from16(bytes, { as });
}
function fromHex6(hex, options = {}) {
  const { as = "Hex" } = options;
  return from16(hex, { as });
}
...
function getEncodableBytes(bytesOrHex) {
  const bytes = typeof bytesOrHex === "string" ? fromHex4(bytesOrHex) : bytesOrHex;
...
var sha2566 = wrapConstructor(() => new SHA2562());
...
function FpSqrt(P) {
  ...
  if (P % _8n === _5n) {
    ...
    return function sqrt5mod8(Fp3, n) {
      ...
      const i2 = Fp3.mul(Fp3.mul(nv, _2n3), v);
...
function weierstrassPoints(opts) {
  ...
  const toBytes12 = CURVE.toBytes || ((_c, point, _isCompressed) => {
    const a = point.toAffine();
    return concatBytes(Uint8Array.from([4]), Fp3.toBytes(a.x), Fp3.toBytes(a.y));
  });
  const fromBytes9 = CURVE.fromBytes || ((bytes) => {
  ...
  class Point2 {
     ...
     */
    static fromHex(hex) {
      const P = Point2.fromAffine(fromBytes9(ensureBytes("pointHex", hex)));
      P.assertValidity();
      return P;
    }
    ...
    toRawBytes(isCompressed = true) {
      ...
      this.assertValidity();
      return toBytes12(Point2, this, isCompressed);
    }
...
function weierstrass(curveDef) {
  ...
  const slcNum = (b2, from35, to3) => bytesToNumberBE(b2.slice(from35, to3));
  class Signature {
...
function hash_to_field(msg, count, options) {
  ...
  const { p, k, m, hash: hash9, expand, DST: _DST } = options;
  abytes2(msg);
  anum(count);
...
function createHasher3(Point2, mapToCurve, def) {
  ...
  return {
    ...
    encodeToCurve(msg, options) {
      ...
      const P = Point2.fromAffine(mapToCurve(u2[0])).clearCofactor();
      P.assertValidity();
      return P;
    },
...
var htf = (() => createHasher3(secp256k1.ProjectivePoint, (scalars) => {
...
function fromBytes5(signature) {
  return fromHex7(fromBytes2(signature));
}
function fromHex7(signature) {
...
function from17(signature) {
  const signature_ = (() => {
    if (typeof signature === "string")
      return fromHex7(signature);
...
function fromDerHex(signature) {
  const { r, s } = secp256k1.Signature.fromDER(from6(signature).slice(2));
  return { r, s };
}
...
function toBytes7(signature) {
  return fromHex4(toHex7(signature));
}
  ...
  constructor({ signature }) {
    super(`Value \`${signature}\` is an invalid signature size.`, {
      metaMessages: [
        ...
        `Received ${size4(from6(signature))} bytes.`
      ]
    });
    Object.defineProperty(this, "name", {
      enumerable: true,
...
// node_modules/ox/_esm/core/Authorization.js
function from18(authorization, options = {}) {
...
function hash2(authorization) {
  return keccak2563(concat4("0x05", fromHex6(toTuple2(authorization))));
}
...
function from19(value) {
  let bytes = from7(value);
  let integer = (() => {
    let hex = value;
    if (value instanceof Uint8Array)
      hex = fromBytes2(bytes);
    return BigInt(hex);
  })();
  let result = "";
  while (integer > 0n) {
...
function fromHex8(value) {
  return from19(value);
}
function fromString5(value) {
  return from19(fromString4(value));
}
function toBytes8(value) {
  return fromHex4(toHex8(value));
}
...
function fromHex9(value, options = {}) {
  return fromBytes7(fromHex4(value), options);
}
function fromString6(value, options = {}) {
  return fromBytes7(fromString4(value), options);
}
...
function toHex9(value) {
  return fromBytes2(toBytes9(value));
}
function toString5(value) {
  return toString3(toBytes9(value));
}
...
function merkelize(tree) {
  function inner(node) {
    ...
    if (node.type === "internal") {
      ...
      return hash3(concat5(hash_left, hash_right));
    }
    let level = node.values.map(hash3);
    while (level.length > 1) {
      const level_ = [];
      for (let i2 = 0; i2 < level.length; i2 += 2)
        level_.push(hash3(concat5(level[i2], level[i2 + 1])));
      level = level_;
    }
    return hash3(concat5(node.stem, new Uint8Array(1).fill(0), level[0]));
  }
...
function commitmentToVersionedHash(commitment, options = {}) {
  ...
  return as === "Bytes" ? versionedHash : fromBytes2(versionedHash);
}
function from21(data, options = {}) {
  const as = options.as ?? (typeof data === "string" ? "Hex" : "Bytes");
  const data_ = typeof data === "string" ? fromHex4(data) : data;
  ...
  return as === "Bytes" ? blobs.map((x) => x.bytes) : blobs.map((x) => fromBytes2(x.bytes));
}
...
function to2(blobs, to3) {
  ...
  const blobs_ = typeof blobs[0] === "string" ? blobs.map((x) => fromHex4(x)) : blobs;
  ...
  return to_ === "Hex" ? fromBytes2(trimmedData) : trimmedData;
}
...
function toCommitments(blobs, options) {
  ...
  return as === "Bytes" ? commitments : commitments.map((x) => fromBytes2(x));
}
function toProofs(blobs, options) {
  const { kzg } = options;
  const as = options.as ?? (typeof blobs[0] === "string" ? "Hex" : "Bytes");
  const blobs_ = typeof blobs[0] === "string" ? blobs.map((x) => fromHex4(x)) : blobs;
  const commitments = typeof options.commitments[0] === "string" ? options.commitments.map((x) => fromHex4(x)) : options.commitments;
  const proofs = [];
  ...
  return as === "Bytes" ? proofs : proofs.map((x) => fromBytes2(x));
}
...
function contains(bloom, input) {
  const filter = fromHex4(bloom);
  const hash9 = keccak2563(input, { as: "Bytes" });
  for (const i2 of [0, 2, 4]) {
    const bit = hash9[i2 + 1] + (hash9[i2] << 8) & 2047;
...
function bls(CURVE) {
  ...
  const G1 = Object.assign(G1_, createHasher3(G1_.ProjectivePoint, CURVE.G1.mapToCurve, {
    ...CURVE.htfDefaults,
    ...CURVE.G1.htfDefaults
  }));
  const G2_ = weierstrassPoints({ n: Fr2.ORDER, ...CURVE.G2 });
  const G2 = Object.assign(G2_, createHasher3(G2_.ProjectivePoint, CURVE.G2.mapToCurve, {
    ...CURVE.htfDefaults,
...
function psiFrobenius(Fp3, Fp22, base2) {
  ...
  const PSI_Y = Fp22.pow(base2, (Fp3.ORDER - _1n8) / _2n7);
  function psi(x, y2) {
    const x2 = Fp22.mul(Fp22.frobeniusMap(x, 1), PSI_X);
    const y22 = Fp22.mul(Fp22.frobeniusMap(y2, 1), PSI_Y);
    return [x2, y22];
  }
  const PSI2_X = Fp22.pow(base2, (Fp3.ORDER ** _2n7 - _1n8) / _3n4);
  const PSI2_Y = Fp22.pow(base2, (Fp3.ORDER ** _2n7 - _1n8) / _2n7);
...
var Fr = Field2(BigInt("0x73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000001"));
  ...
  G2: {
    ...
    fromBytes: (bytes) => {
      ...
      if (value.length === 96 && compressed) {
        ...
        const right = Fp2.add(Fp2.pow(x, _3n5), b2);
        let y2 = Fp2.sqrt(right);
        const Y_bit = y2.c1 === _0n9 ? y2.c0 * _2n8 / P : y2.c1 * _2n8 / P ? _1n9 : _0n9;
    ...
    toBytes: (c, point, isCompressed) => {
      ...
      if (isCompressed) {
        if (isZero)
          return concatBytes(COMPRESSED_ZERO, numberToBytesBE(_0n9, len));
        const flag = Boolean(y2.c1 === _0n9 ? y2.c0 * _2n8 / P : y2.c1 * _2n8 / P);
...
function getPublicKey(options) {
  ...
  const { px, py, pz } = group.ProjectivePoint.fromPrivateKey(from6(privateKey).slice(2));
...
function sign2(options) {
  ...
  const payloadPoint = payloadGroup.hashToCurve(from7(payload), suite ? { DST: fromString4(suite) } : void 0);
...
function verify2(options) {
  ...
  const payloadPoint = group.hashToCurve(from7(payload), suite ? { DST: fromString4(suite) } : void 0);
...
function toHex11(point) {
  return fromBytes2(toBytes11(point));
}
...
function fromHex10(hex, group) {
  return fromBytes8(toBytes3(hex), group);
}
...
function fromCreate(options) {
  const from35 = fromHex4(from9(options.from));
  let nonce = fromNumber3(options.nonce);
  if (nonce[0] === 0)
    nonce = new Uint8Array([]);
  return from9(`0x${keccak2563(fromBytes4([from35, nonce], { as: "Hex" })).slice(26)}`);
}
function fromCreate2(options) {
  const from35 = fromHex4(from9(options.from));
  const salt = padLeft3(validate3(options.salt) ? options.salt : fromHex4(options.salt), 32);
  const bytecodeHash = (() => {
    if ("bytecodeHash" in options) {
      if (validate3(options.bytecodeHash))
        return options.bytecodeHash;
      return fromHex4(options.bytecodeHash);
    }
    return keccak2563(options.bytecode, { as: "Bytes" });
  })();
  return from9(slice3(keccak2563(concat5(fromHex4("0xff"), from35, salt, bytecodeHash), { as: "Hex" }), 12));
}
...
function labelhash(label) {
  ...
  if (!label)
    return fromBytes2(result);
  return unwrapLabelhash(label) || keccak2563(fromString3(label));
}
function namehash(name) {
  ...
  for (let i2 = labels.length - 1; i2 >= 0; i2 -= 1) {
    ...
    const hashed = hashFromEncodedLabel ? fromHex4(hashFromEncodedLabel) : keccak2563(fromString4(labels[i2]), { as: "Bytes" });
    result = keccak2563(concat5(result, hashed), { as: "Bytes" });
  }
  return fromBytes2(result);
}
...
function getPublicKey2(options) {
  ...
  const point = secp256k1.ProjectivePoint.fromPrivateKey(from6(privateKey).slice(2));
  return from8(point);
}
...
function recoverPublicKey(options) {
  ...
  const point = signature_.recoverPublicKey(from6(payload).substring(2));
  return from8(point);
}
function sign3(options) {
  const { extraEntropy: extraEntropy3 = extraEntropy2, hash: hash9, payload, privateKey } = options;
  const { r, s, recovery } = secp256k1.sign(from7(payload), from7(privateKey), {
    extraEntropy: typeof extraEntropy3 === "boolean" ? extraEntropy3 : from6(extraEntropy3).slice(2),
    lowS: true,
    ...hash9 ? { prehash: true } : {}
  });
  return {
...
function verify3(options) {
  ...
  return secp256k1.verify(signature, from7(payload), toBytes4(publicKey), ...hash9 ? [{ prehash: true, lowS: true }] : []);
}
...
function fromScure(key) {
  return {
    ...
    depth: key.depth,
    identifier: fromBytes2(key.identifier),
    index: key.index,
    privateKey: fromBytes2(key.privateKey),
...
function fromSeed(seed, options = {}) {
  ...
  const key = HDKey.fromMasterSeed(from7(seed), versions);
  return fromScure(key);
}
...
function encode8(data) {
  const message = from6(data);
  return concat4(
    // Personal Sign Format: `0x19 ‖ "Ethereum Signed Message:\n" ‖ message.length ‖ message`
    "0x19",
    fromString3("Ethereum Signed Message:\n" + size4(message)),
    message
  );
}
function getSignPayload4(data) {
  return keccak2563(encode8(data));
}
...
function from23(response, options = {}) {
...
function from24(provider, options = {}) {
  var _a, _b;
...
var Fp2562 = Field2(BigInt("0xffffffff00000001000000000000000000000000ffffffffffffffffffffffff"));
var CURVE_A = Fp2562.create(BigInt("-3"));
var CURVE_B = BigInt("0x5ac635d8aa3a93e7b3ebbd55769886bc651d06b0cc53b0f63bce3c3e27d2604b");
var p2563 = createCurve2({
  a: CURVE_A,
  // Equation params: a, b
  b: CURVE_B,
  Fp: Fp2562,
...
var secp256r13 = p2563;
var mapSWU2 = (() => mapToCurveSimpleSWU2(Fp2562, {
  A: CURVE_A,
  B: CURVE_B,
  Z: Fp2562.create(BigInt("-10"))
}))();
var htf2 = (() => createHasher3(secp256r13.ProjectivePoint, (scalars) => mapSWU2(scalars[0]), {
  DST: "P256_XMD:SHA-256_SSWU_RO_",
  encodeDST: "P256_XMD:SHA-256_SSWU_NU_",
  p: Fp2562.ORDER,
  m: 1,
...
var hashToCurve3 = (() => htf2.hashToCurve)();
var encodeToCurve3 = (() => htf2.encodeToCurve)();

// node_modules/ox/_esm/core/P256.js
var noble3 = secp256r13;
function getPublicKey3(options) {
  const { privateKey } = options;
  const point = secp256r13.ProjectivePoint.fromPrivateKey(typeof privateKey === "string" ? privateKey.slice(2) : fromBytes2(privateKey).slice(2));
  return from8(point);
}
function randomPrivateKey3(options = {}) {
  const { as = "Hex" } = options;
  const bytes = secp256r13.utils.randomPrivateKey();
  if (as === "Hex")
    return fromBytes2(bytes);
  return bytes;
}
function recoverPublicKey2(options) {
  const { payload, signature } = options;
  const { r, s, yParity } = signature;
  const signature_ = new secp256r13.Signature(BigInt(r), BigInt(s)).addRecoveryBit(yParity);
  const payload_ = payload instanceof Uint8Array ? fromBytes2(payload) : payload;
  const point = signature_.recoverPublicKey(payload_.substring(2));
  return from8(point);
}
function sign4(options) {
  const { extraEntropy: extraEntropy3 = extraEntropy2, hash: hash9, payload, privateKey } = options;
  const { r, s, recovery } = secp256r13.sign(payload instanceof Uint8Array ? payload : fromHex4(payload), privateKey instanceof Uint8Array ? privateKey : fromHex4(privateKey), {
    extraEntropy: typeof extraEntropy3 === "boolean" ? extraEntropy3 : from6(extraEntropy3).slice(2),
    lowS: true,
    ...hash9 ? { prehash: true } : {}
  });
...
function verify4(options) {
  ...
  return secp256r13.verify(signature, payload instanceof Uint8Array ? payload : fromHex4(payload), toHex5(publicKey).substring(2), ...hash9 ? [{ prehash: true, lowS: true }] : []);
}
...
function createMessage(value) {
  ...
  const address = from9(value.address, { checksum: true });
  const origin = (() => {
    if (scheme)
...
function from27(value, decimals = 0) {
...
function fromEther(ether, unit = "wei") {
  return from27(ether, exponents.ether - exponents[unit]);
}
function fromGwei(gwei, unit = "wei") {
  return from27(gwei, exponents.gwei - exponents[unit]);
}
...
function serialize3(envelope, options = {}) {
  ...
  return concat4(serializedType, fromHex6(serialized));
}
...
function from30(envelope, options = {}) {
  const { signature } = options;
...
function serialize4(envelope, options = {}) {
  ...
  return concat4("0x01", fromHex6(serialized));
}
...
function from31(envelope, options = {}) {
  const { signature } = options;
...
function from32(envelope, options = {}) {
  const { signature } = options;
...
function serialize6(envelope, options = {}) {
  ...
  return concat4(serializedType4, fromHex6(serialized));
}
...
function serialize7(value) {
  ...
  const domain = (() => {
    if (!domain_)
      return {};
    const type6 = types2.EIP712Domain ?? extractEip712DomainTypes2(domain_);
...
var BytesSizeMismatchError3 = class extends BaseError3 {
  constructor({ expectedSize, givenSize }) {
    super(`Expected bytes${expectedSize}, got bytes${givenSize}.`);
    Object.defineProperty(this, "name", {
      enumerable: true,
...
function encodeData3(value) {
  ...
  return encode4(encodedTypes, encodedValues);
}
function hashType2(value) {
  const { primaryType, types: types2 } = value;
  const encodedHashType = fromString3(encodeType3({ primaryType, types: types2 }));
...
function encodeField2(properties) {
  ...
  if (types2[type6] !== void 0)
    return [
      { type: "bytes32" },
      keccak2563(encodeData3({ data: value, primaryType: type6, types: types2 }))
    ];
  if (type6 === "bytes") {
    const prepend = value.length % 2 ? "0" : "";
    value = `0x${prepend + value.slice(2)}`;
  ...
  if (type6 === "string")
    return [
      { type: "bytes32" },
      keccak2563(fromString4(value), { as: "Hex" })
    ];
  if (type6.lastIndexOf("]") === type6.length - 1) {
    const parsedType = type6.slice(0, type6.lastIndexOf("["));
    ...
    return [
      { type: "bytes32" },
      keccak2563(encode4(typeValuePairs.map(([t]) => t), typeValuePairs.map(([, v]) => v)))
    ];
  }
  return [{ type: type6 }, value];
}
...
function getSignPayload11(value) {
  return keccak2563(encode10(value));
}
...
function parseAsn1Signature2(bytes) {
  ...
  return {
    r,
    s: s > p2563.CURVE.n / 2n ? p2563.CURVE.n - s : s
  };
}
...
function getClientDataJSON(options) {
  ...
  return JSON.stringify({
    type: "webauthn.get",
    challenge: fromHex9(challenge2, { url: true, pad: false }),
    origin,
    crossOrigin,
    ...extraClientData
  });
...
function getCredentialCreationOptions(options) {
  ...
  return {
    publicKey: {
      ...
      rp,
      user: {
        id: (user == null ? void 0 : user.id) ?? keccak2563(fromString4(name), { as: "Bytes" }),
        name,
...
function verify5(options) {
  ...
  if (fromBytes2(toBytes9(challenge_extracted)) !== challenge2)
    return false;
...
async function createKeyPair(options = {}) {
  ...
  const publicKey = from8(new Uint8Array(publicKey_raw));
  return {
...
async function verify6(options) {
  ...
  return await globalThis.crypto.subtle.verify({
    name: "ECDSA",
    hash: "SHA-256"
  }, publicKey, concat5(fromNumber3(signature.r), fromNumber3(signature.s)), from7(payload));
}
...
function createWalletSendCallsRequest({ calls, from: from35, chainId, capabilities }) {
...
@noble/curves/esm/p256.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=dist-GOLZWCAW.js.map
